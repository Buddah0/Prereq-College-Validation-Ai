...
def check_missing_prereqs(graph: nx.DiGraph, real_course_ids: Set[str]) -> List[Issue]:
    ...
    for node in graph.nodes():
        if node not in real_course_ids:
            ...
            )  # node -> successors means node is prereq for successors
            issues.append(
                Issue(
                    code="missing_prereq",
                    severity="high",
...
def check_longest_chain(graph: nx.DiGraph) -> tuple[List[Issue], int, List[str], bool]:
    ...
    try:
        ...
        if (
            length > 6
        ):  # Configurable threshold, hardcoded for now or passed via config?
            issues.append(
                Issue(
                    code="long_chain",
...
def write_issues_csv(report: Report, out_path: str):
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for issue in report.issues:
            writer.writerow(
                [issue.severity, issue.code, ",".join(issue.courses), issue.message]
            )
