...
def format_cycles_text(res: CyclesResult) -> str:
    ...
    if res.total_count == 0:
        ...
        lines.append(f"Detected {res.total_count} cycle(s). showing first {res.shown_count}:")
...
def _base_json_wrapper(json_path: str, cmd: str, has_cycles: bool, skipped: List[str], data: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "schema_version": "1",
    ...
    while i < len(argv):
        ...
        if val_found is not None:
            if json_val is not None and json_val != val_found:
                print("Error: Multiple different --json arguments provided.", file=sys.stderr)
                sys.exit(2)
            json_val = val_found
            i += consumed
        else:
        ...
        if cmd == "report":
            ...
            if has_cycles:
                ...
                if fmt_json:
                    ...
                    wrapper = _base_json_wrapper(real_json_path, cmd, True, skipped, data)
                    final_output = json.dumps(wrapper, sort_keys=True, indent=2)
                else:
                    final_output = format_report_text(real_json_path, stats, cycles_res, None, None)
            else:
                ...
                if fmt_json:
                    ...
                    final_output = format_report_text(real_json_path, stats, cycles_res, chain_res, unlocked_res)
            ...
            if fmt_json:
                ...
                wrapper = _base_json_wrapper(real_json_path, cmd, cycles_res.total_count > 0, [], data)
                final_output = json.dumps(wrapper, sort_keys=True, indent=2)
            else:
            ...
            if not args.course:
                print("Error: --course is required for chain subcommand", file=sys.stderr)
                return 2
            ...
            if not args.completed:
                 print("Error: --completed is required for unlocked subcommand", file=sys.stderr)
                 return 2
...
def _build_parser():
    ...
    # REPORT
    p_report = subparsers.add_parser("report", help="Show full analysis report (default)")
    p_report.add_argument("--course", help="Target course for example chain")
    p_report.add_argument("--completed", nargs="+", help="Completed course IDs for unlocked check")
    p_report.add_argument("--max-cycles", type=int, default=5, help="Max cycles to display")
    ...
    p_cycles.add_argument("--max-cycles", type=int, default=5, help="Max cycles to display")
    ...
    p_unlocked.add_argument("--completed", nargs="+", required=True, help="Completed course IDs")
